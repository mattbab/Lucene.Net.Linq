...
namespace Lucene.Net.Linq
{
    ...
    public static class LuceneMethods
    {
        ...
        /// <summary>
        /// Instructs the query parser that a given query pattern in a LINQ where clause should not have special
        /// characters (such as <c>*</c>) escaped.
        ///
        /// Disabling escaping allows prefix, wildcard, phrase and range queries to be parsed from the <paramref
        /// name="queryPattern" /> instead of treating it as a verbatim search term.
        /// </summary>
        /// <example>
        /// The following two samples will produce the same <c cref="Query">Query</c>: <c>var query = "Foo*";
        ///
        /// var results = from doc in documents where doc.Title == query.AllowSpecialCharacters() select doc; </c><c>
        /// var query = "Foo";
        ///
        /// var results = from doc in documents where doc.Title.StartsWith(query) select doc; </c>
        /// </example>
        ...
        /// <summary>
        /// Applies a custom boost function to customize query scoring. When multiple boost functions are added by
        /// calling this method more than once, the return values from each function are multiplied to yield a final
        /// result. Warning: this method will cause each document that matches the query to be converted to an instance
        /// of <typeparamref name="T" /> in order for the score to be computed, significantly degrading performance.
        /// </summary>
        ...
        /// <summary>
        /// Applies the provided Query. Enables queries to be constructed from outside of LINQ to be executed as part of
        /// a LINQ query.
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Applies a custom boost function to customize query scoring. When multiple boost functions are added by
        /// calling this method more than once, the return values from each function are multiplied to yield a final result.
        /// </summary>
